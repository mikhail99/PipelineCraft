...
import { RefreshCw, ArrowUpRight, ArrowDownRight, Pencil, Check, X, FileText, Table as TableIcon, Loader2, Network, ChevronRight, Save } from 'lucide-react';
...
export default function EntityWorkspace({ theme = 'dark' }) {
    const { activeEntity, entities, updateEntity, getParents, getChildren, recomputeEntity } = usePipeline();
    ...
    // Reset chain editing when entity changes
    useEffect(() => {
    ...
    const handleStartEditChain = () => {
        setEditingChain(activeEntity.config?.operationChain || []);
        setChainModified(false);
    };
    ...
    const handleSaveChain = () => {
        ...
        const pipelineDescription = editingChain.map(s => s.operation).join(' → ');

        // Collect all secondary inputs
        const additionalDeps = editingChain
            .filter(s => s.secondaryInput)
            .map(s => s.secondaryInput);

        const currentDeps = activeEntity.dependencies || [];
        const primaryDep = currentDeps[0]; // Keep the primary input
        const newDeps = primaryDep ? [primaryDep, ...additionalDeps] : additionalDeps;
        ...
        updateEntity({
            ...
            data: {
                config: {
                    ...activeEntity.config,
                    operationChain: editingChain,
                    pipelineDescription
                },
                dependencies: [...new Set(newDeps)], // Remove duplicates
                status: 'stale' // Mark as stale since pipeline changed
            }
        });
        setEditingChain(null);
        setChainModified(false);
                ...
                {activeTab === 'graph' ? (
                    ...
                    <div className="p-6">
                        <div className="space-y-6 max-w-4xl">
                            ...
                            {/* Operation Chain Display/Edit */}
                            {(activeEntity.config?.operationChain && activeEntity.config.operationChain.length > 0) || editingChain ? (
                                <div>
                                    <div className="flex items-center justify-between mb-3">
                                        <label className={cn(
                                            ...
                                            Operation Pipeline ({(editingChain || activeEntity.config?.operationChain || []).length} step{(editingChain || activeEntity.config?.operationChain || []).length !== 1 ? 's' : ''})
                                        </label>
                                        {!editingChain ? (
                                            <Button
                                                variant="ghost"
                                                size="sm"
                                                onClick={handleStartEditChain}
                                                className={cn(
                                                    "h-7 gap-1.5",
                                                    theme === 'light' ? 'text-gray-600 hover:text-gray-800' : 'text-slate-400 hover:text-slate-200'
                                                )}
                                            >
                                            ...
                                            <div className="flex gap-2">
                                                ...
                                                <Button
                                                    ...
                                                    disabled={!chainModified}
                                                    className={cn(
                                                        "h-7 gap-1.5",
                                                        theme === 'light' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-cyan-600 hover:bg-cyan-700'
                                                    )}
                                                >
                                    ...
                                    {editingChain ? (
                                        <div className={cn(
                                            ...
                                            theme === 'light' ? 'bg-gray-50 border-gray-200' : 'bg-slate-800/30 border-slate-700/50'
                                        )}>
                                            <OperationChainBuilder
                                                steps={editingChain}
                                                onStepsChange={handleChainChange}
                                                entities={entities}
                                                theme={theme}
                                            />
                                        </div>
                                        ...
                                        <div className={cn(
                                            ...
                                            <div className="flex flex-wrap items-center gap-2">
                                                {activeEntity.config.operationChain.map((step, idx) => {
                                                    ...
                                                    return (
                                                        <div key={step.id} className="flex items-center gap-2">
                                                            <div className={cn(
                                                                ...
                                                                theme === 'light' ? 'bg-white border-gray-200' : 'bg-slate-900/50 border-slate-600'
                                                            )}>
                                                                <span className={cn(
                                                                ...
                                                                {step.params && Object.keys(step.params).length > 0 && (
                                                                    <span className={cn(
                                                                        ...
                                                                        ({Object.values(step.params).filter(Boolean).join(', ')})
                                                                    </span>
                                                                )}
                                                            </div>
                                                            {idx < activeEntity.config.operationChain.length - 1 && (
                            ...
                            {/* Legacy single operation display */}
                            {activeEntity.config?.operationName && !activeEntity.config?.operationChain && (
                                <div>
